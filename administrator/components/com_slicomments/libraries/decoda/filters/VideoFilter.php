...
class VideoFilter extends DecodaFilter {
	...
	public function parse(array $tag, $content) {
		$provider = isset($tag['attributes']['default']) ? $tag['attributes']['default'] : 'youtube';
		$size = strtolower(isset($tag['attributes']['size']) ? $tag['attributes']['size'] : 'small');

		if (empty($this->_formats[$provider])
		|| ($this->_formats[$provider]['pattern'] && !preg_match($this->_formats[$provider]['pattern'], $content))) {
			return htmlentities($provider .':'. $content, ENT_QUOTES, 'UTF-8');
		}
