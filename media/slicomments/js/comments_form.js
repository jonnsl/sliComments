window.addEvent('domready', function(){
	$$('.comments_form.no-js').removeClass('no-js');
		...
		var textarea = document.id('comments_form_textarea');
		var counter = document.id('comments-remaining-count');
		var update_counter = function() {
			...
			if (remaining_chars <= 5) {
				...
				if (remaining_chars < 0) {
					this.form.getElement('button').set('disabled', true)
				}
			} else {
				this.form.getElement('button').set('disabled', false)
		...
		if (placeholder_support) {
			...
			new OverText(document.id('comments_form_name'));
			new OverText(document.id('comments_form_email'));
		}
		
		document.id('comments_form_send').addEvent('click', function(e){
			e.stop();
			var form = this.form,
			...
			new Request({
				...
				method: 'post',
				data: 'article_id='+this.form.article_id.value+'&name='+this.form.name.value+'&email='+this.form.email.value+'&text='+this.form.text.value+'&'+this.form.getElement('input[type=hidden]').get('name')+'=1',
				onRequest: function(){
				...
				onSuccess: function(responseText){
					...
					if (response.success) {
						...
						}).inject(document.id('comments_list'), 'top');
